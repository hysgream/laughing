...
ENV=$1
if [ -z "$ENV" ]; then
    echo "ERROR:PELEASE SPEC ENV ARGS,SUCH AS test,develop..  "
    exit 1
fi
cd `dirname $0`
BIN_DIR=`pwd`
...
SERVER_NAME=`sed '/dubbo.application.name/!d;s/.*=//' conf/$ENV/dubbo.properties | tr -d '\r'`
SERVER_PORT=`sed '/dubbo.protocol.port/!d;s/.*=//' conf/$ENV/dubbo.properties | tr -d '\r'`
LOGS_FILE=`sed '/dubbo.log4j.file/!d;s/.*=//' conf/$ENV/dubbo.properties | tr -d '\r'`
...
JAVA_OPTS="-Xbootclasspath/a:.  -Denv=$ENV -Ddubbo.properties.file=$DUBBO_FILE -Djava.awt.headless=true -Djava.net.preferIPv4Stack=true "
JAVA_DEBUG_OPTS=""
if [ "$1" = "debug" ]; then
