...
import com.laughing.lang.exceptions.TooManyMethodMatchesException;
import com.laughing.lang.magic.config.MagicFactory;
import com.laughing.lang.magic.reflect.ReflectMagicClass;
import com.laughing.lang.magic.reflect.ReflectMagicMethod;
 ...
 * class魔术包装类，提供了简单方便的class操作， 实现上根据环境分为反射实现和cglib实现。 <br />
 * <p/>
...
public abstract class MagicClass implements Serializable {
	...
	protected Method getMethodByName(String methodName) {
		...
		if (ret.size() > 1) {
			throw new TooManyMethodMatchesException("too many methods find by name:" + methodName + " are"
					+ SystemUtil.LINE_SEPARATOR + ret.join(SystemUtil.LINE_SEPARATOR));
		}
		return ret.get(0);
	}
	...
	protected Map<String, Property> findPropertiesFromClass() {
		...
		for (Map.Entry<String, Method> entry : readableMethods.entrySet()) {
			...
			Property property = new Property(name, propertyClass, new ReflectMagicMethod(readMethod),
					(writeMethod != null ? new ReflectMagicMethod(writeMethod) : null));
